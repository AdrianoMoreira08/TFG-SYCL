...
TEST_CASE("managing_dependencies_usm", "managing_dependencies_source") {
  ...
  try {
    ...
    sycl::queue compute_queue{
      sycl::aspect_selector(sycl::aspect::usm_device_allocations), AsyncHandler};
    ...
    auto initialization = compute_queue
    .parallel_for(sycl::range{data_size}, [=](sycl::id<1> index) {
    ...
    .parallel_for(sycl::range{data_size}, initialization, [=](sycl::id<1> index) {
    ...
    auto kernel_c = compute_queue
    .parallel_for(sycl::range{data_size}, kernel_a, [=](sycl::id<1> index) {
      in_c[index] -= in_a[index];
    });
    auto kernel_d = compute_queue
    .parallel_for(sycl::range{data_size}, {kernel_b, kernel_c}, [=](sycl::id<1> index) {
...
// Task: Run these kernels on the SYCL device, respecting the dependencies
// as shown in the README
TEST_CASE("managing_dependencies_buffer_acc", "managing_dependencies_source") {
  auto AsyncHandler = [](sycl::exception_list exceptions) {
    for (auto& exception: exceptions) {
      try {
        std::rethrow_exception(exception);
  ...
  try {
    ...
    sycl::queue compute_queue{AsyncHandler};
